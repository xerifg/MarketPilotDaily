...
当前部署版本：`96393671-46c5-4e23-b426-335d522aeece`（2026-09-16）。D1 已应用 `0001`–`0006` 迁移，外键检查通过；发送开关保存为 `email_paused=0`。每日只自动使用版本 1，人工测试可显式选择版本 1–3，不覆盖已发报告。任务身份使用 GitHub OIDC；无需另外保存长期机器凭证。

2026-09-16 更新：邮件改为重点摘要，完整内容保留在网站；新增行业资金、沪市 ETF 估算净申赎及持仓关联模块，范围与口径见 [资金流向模块](fund-flows.md)。新版每日任务北京时间 05:00 启动，目标 05:15 发送，实际时间受调度延迟影响。Python 53 项、TypeScript 38 项测试及生产构建通过；桌面和手机布局、邮件摘要完成检查。本次发布不触发测试邮件、不补写历史日报，无新增数据库迁移。下文为早期开发记录，其中未接入项目以本条更新为准。
