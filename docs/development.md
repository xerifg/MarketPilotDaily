...
当前部署版本：`e2e7c59c-434b-437a-8b29-f6c263b1d07c`。D1 已应用 `0001`–`0006` 迁移，外键检查通过；发送开关保存为 `email_paused=0`。每日只自动使用版本 1，人工测试可显式选择版本 1–3，不覆盖已发报告。任务身份使用 GitHub OIDC；无需另外保存长期机器凭证。
